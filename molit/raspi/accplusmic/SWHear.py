...
#    return freq[:int(len(freq)/2)],fft[:int(len(fft)/2)]
	return freq,fft
class SWHear(threading.Thread):
	def __init__(self,device=None,rate=None,updatesPerSecond=10):
		threading.Thread.__init__(self)
		self.p=pyaudio.PyAudio()
		self.chunk=9192 # gets replaced automatically
	...
	def setting(self, count):
		lastRead=self.chunksRead
		while lastRead==self.chunksRead:
			time.sleep(.01)
		temp=self.fft
		temp[0:10]=0
		temp[4400:4410]=0
		temp=np.fft.ifft(temp)
		self.th=max(np.mean(np.abs(temp)),self.th)
		if count%5==0:
			print("mic", count/5, "sec left")
			print("th:",self.th)
		lastRead=self.chunksRead
		count-=1
		timer=threading.Timer(0.01,self.setting, args=[count])
		if count>0:
			timer.start()
		else:
			self.th=1.5*self.th
	...
	def run(self):
		while True:
			...
			temp[4400:4410]=0
			temp=np.fft.ifft(temp)
			self.value=np.mean(np.abs(temp))
			if self.value>self.th:
				self.msg="Noise!"
			lastRead=self.chunksRead
	#		timer=threading.Timer(0.01, self.exam)
	#		timer.start()
